...
def run_code(file_tag,dir_name,analysis_type='Full'):
    ...
    Ea_by_R.to_csv('Ea_by_R_'+str(analysis_type)+'_'+str(file_tag)+'.csv')
    ...
    plt.title('Ignition delay plot of alkanes C3-C16 \n $[P='+str(file_tag)+'atm , X_{Fuel}=1\%, X_{Oxi}=Stoichiometric, X_{Ar}=remaining]$')
    plt.xlabel('1000/T [K]')
    ...
    plt.savefig('./plots/combined_'+str(analysis_type)+'_'+str(file_tag)+'.eps', dpi=200)
    plt.show()
...
######################    INPUT     #############################
#################################################################
file_tag = 5 #define pressure here
dir_name = 'chemkin_all_5' #define directory you want to analyze
#############        Output file name         ###################
analysis_type = 'Half' #Full or Half ... which stand for higher temperature
#################################################################
#################################################################
