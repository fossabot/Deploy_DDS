...
# This  file will call whole training set and based using saved object classify the data 
...
class training_accuracy_check():
        ...
        def training_accuracy(self,data):
                '''
                Adding train set as external source to predict ID 
                '''
                ...
                df,tau = Sel_feat.feature_selection(data)
                ...
                for i in range(num_of_clusters):
                        ...
                        #As Testing of all data points, all data points are assigned to the X_train
                        #testing to make compatible with dataframe 
                        X_train, y_train = cluster_dataframe.to_numpy() , y_cluster.to_numpy()
                        # print('X_train: ', X_train)
                        # print(regressor_OLS_modified.params)

                        y_pred = regressor_OLS_modified.predict(X_train)
                        cluster_dataframe['Class'] = cluster_dataframe_class
                        cluster_dataframe['Time(μs)_actual'] = y_train
                        cluster_dataframe['Time(μs)_predicted'] = y_pred
                        # result = loaded_model.score(X_test, Y_test)

                        SF.check_directory(str(self.curr_directory)+'/training_accuracy_result/console_output/') #checking directory
                        SF.check_directory(str(self.curr_directory)+'/training_accuracy_result/classified_data/') #checking directory
                        SF.check_file_existence(str(self.curr_directory)+"/training_accuracy_result/console_output/output_result.txt")
                        cluster_dataframe.to_csv(str(self.curr_directory)+'/training_accuracy_result/classified_data/classified_cluster'+str(cluster_label[i])+'.csv')
                        ...
                        ID_comparison['y_actual'] = y_train
                        ID_comparison['Relative Error'] = np.abs(y_pred - y_train)/np.abs(y_train)
                        # calc rmse
                        rmse_val = rmse(y_train, y_pred)
                        SF.check_directory(str(self.curr_directory)+'/training_accuracy_result/Ignition_delay_comparison/') #checking directory
                        ID_comparison.to_csv(str(self.curr_directory)+'/training_accuracy_result/Ignition_delay_comparison/ID_comparison_train_cluster_'+str(cluster_label[i])+'.csv')
                        maximum_relative_error_train = self.max_relative_error(y_train,y_pred)
                        f.write('\n\n Maximum Relative Error in training data for cluster-'+str(cluster_label[i])+' :'+str(maximum_relative_error_train))
                        print('\n\n Maximum Relative Error in training data for cluster-', str(cluster_label[i]),' :',str(maximum_relative_error_train))
                        ...
                        SF.check_directory(str(self.curr_directory)+'/training_accuracy_result/error_frequency/') #checking directory
                        plt.clf()
                        plt.bar(x,y)
                        ...
                        # plt.savefig(str(self.curr_directory)+'/training_accuracy_result/error_frequency/error_frequency_'+str(cluster_label[i])+'.eps')
                        # plt.show()
                        plt.close()
                        ...
                        plt.scatter(y_pred, y_train, s=20, cmap='viridis',label= str('Cluster-'+str(cluster_label[i]) +' data points'))
                        text = "Maximum Relative Error : "+ str(maximum_relative_error_train)
                        plt.xlim([2,11])
                        plt.ylim([2,11])
                        plt.rc('text', usetex=True)
                        ...
                        # plt.savefig(str(self.curr_directory)+'/training_accuracy_result/prediction_comparison_plots/ignition_delay_training_'+str(cluster_label[i])+'.eps', format='eps', dpi=600)
                        plt.close()
                ...
                plt.ylabel('Frequency of Error',fontsize=15)
                plt.savefig(str(self.curr_directory)+'/training_accuracy_result/error_frequency/error_frequency_all_data.jpg', dpi=600)
                # plt.show()
                plt.close()                       
                # plt.show()
                plt.close()
                ...
                #Storing to the file 
                SF.check_file_existence(str(self.curr_directory)+'/training_accuracy_result/error_frequency/error_frequency.csv')
                ...
                error_freq.to_csv(str(self.curr_directory)+'/training_accuracy_result/error_frequency/error_frequency.csv',index=False)
...
if __name__ == "__main__": 
        data = pd.read_csv(str(Main_folder_dir)+'/data/test_set/test_dataset.csv')
        training_accuracy_check.training_accuracy(data)
        # #manual result 
        ...
        # external_test.manual_testing(data,all_cluster_coef)
